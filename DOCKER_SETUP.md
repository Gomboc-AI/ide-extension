...
docker run --rm gombocai/orl:v1.3.9-latest --help

# Test remediation command
docker run --rm gombocai/orl:v1.3.9-latest remediate --help

# Test rules pull command
docker run --rm gombocai/orl:v1.3.9-latest rules pull --help
```
...
docker run --rm \
  ...
  gombocai/orl:v1.3.9-latest remediate /workspace --dry-run
```
...
docker run --rm \
  ...
  gombocai/orl:v1.3.9-latest rules pull \
...
docker run --rm -u $(id -u):$(id -g) -v $(pwd):/workspace gombocai/orl:v1.3.9-latest --help
```
...
docker run --rm gombocai/orl:v1.3.9-latest ping -c 3 google.com
...
services:
  orl:
    image: gombocai/orl:v1.3.9-latest
    volumes:
      - ./workspace:/workspace
      - ./rules:/rules
    environment:
