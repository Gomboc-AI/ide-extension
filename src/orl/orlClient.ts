  ...
  async remediate(
    ...
    try {
      ...
      try {
        ...
        // - code 1: recoverable failure / partial remediation -> treat as success when we have output
        // - code 2: violations found (legacy behavior)        -> treat as success when we have output
        if ((error.code === 1 || error.code === 2) && error.stdout) {
          ...
          const looksLikeRuleLoadFailure = (text: string): boolean => {
            ...
            // Heuristics: ORL failures while loading/parsing rules/rulesets.
            return (
          ...
          const ruleLoadFailure =
            exitCode === 1 && looksLikeRuleLoadFailure(stderr);
          ...
          return {
            ...
            report: reportFile || error.stdout,
            exitCode,
            // Include a short error message when ORL signaled a recoverable rule-load failure.
            // This allows downstream reporting (integrations) without blocking the scan.
