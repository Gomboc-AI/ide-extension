...
export async function testOrlConnectionCommand(context: vscode.ExtensionContext) {
  try {
    ...
    await vscode.window.withProgress({
      ...
      progress.report({ increment: 50, message: 'Testing Docker connectivity...' });
      ...
      if (isConnected) {
        vscode.window.showInformationMessage(
          ...
          if (selection === 'Open Settings') {
            vscode.commands.executeCommand('workbench.action.openSettings', 'gomboc-vscode-extension.remediateOrlEnabled');
          }
        });
      } else {
        vscode.window.showErrorMessage(
          ...
          if (selection === 'Open Settings') {
            vscode.commands.executeCommand('workbench.action.openSettings', 'gomboc-vscode-extension');
          }
        });
      }
    });
