...
const RULE_NAME =
  'gomboc-ai/java/jca/cwe-321-remove-hardcoded-cryptographic-keys';
const IAC_FILE = path.join(CODE_DIR, 'java/hardcoded-key.java');
const WORKSPACE_FILE_KEY = '/workspace/hardcoded-key.java';

describe('Java integration — hardcoded cryptographic key rule', () => {
  let tempDir = '';
  let storageDir = '';
  ...
  it('Tier 1: replaces hardcoded Java key with environment lookup', async () => {
    const result = await runSingleRuleFix();
    expect(result.success).toBe(true);

    const modified = result.modifiedFiles[WORKSPACE_FILE_KEY];
    expect(modified).toBeDefined();
    expect(modified).toContain('System.getenv("USER_INPUT_1")');
    expect(modified).toContain('SecretKeySpec');
  });
  ...
  it('Tier 3: converter emits diagnostic payload for Java file', async () => {
    const result = await runSingleRuleFix();
    const scanResponse = await OrlResultConverter.convertToScanResponse(
      result,
      'java',
      path.join(tempDir, 'hardcoded-key.java'),
    );
