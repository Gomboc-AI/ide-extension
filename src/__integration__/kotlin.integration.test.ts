...
const RULE_NAME = 'gomboc-ai/java/log4shell_upgrade_log4j_gradle_kotlin';
const IAC_FILE = path.join(CODE_DIR, 'kotlin/build.gradle.kts');
const WORKSPACE_FILE_KEY = '/workspace/build.gradle.kts';

describe('Kotlin integration — log4j upgrade rule', () => {
  let tempDir = '';
  let storageDir = '';
  ...
  it('Tier 1: upgrades log4j version in build.gradle.kts', async () => {
  ...
  it('Tier 3: converter emits diagnostic payload for Kotlin file', async () => {
    const result = await runSingleRuleFix();
