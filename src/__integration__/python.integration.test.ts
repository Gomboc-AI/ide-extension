...
const RULE_NAME =
  'gomboc-ai/python/hashlib/cwe-327-replace-broken-hash-algorithms';
const IAC_FILE = path.join(CODE_DIR, 'python/hashlib-md5.py');
const WORKSPACE_FILE_KEY = '/workspace/hashlib-md5.py';

describe('Python integration — hashlib broken algorithm rule', () => {
  let tempDir = '';
  let storageDir = '';
  ...
  it('Tier 1: replaces md5 call with sha256', async () => {
    const result = await runSingleRuleFix();
    const modified = result.modifiedFiles[WORKSPACE_FILE_KEY];
    expect(modified).toContain('hashlib.sha256');
    expect(modified).not.toContain('hashlib.md5');
  });
  ...
  it('Tier 3: converter emits diagnostic payload for Python file', async () => {
    const result = await runSingleRuleFix();
    const scanResponse = await OrlResultConverter.convertToScanResponse(
      result,
      'python',
      path.join(tempDir, 'hashlib-md5.py'),
    );
