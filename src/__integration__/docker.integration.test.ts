...
const RULE_NAME =
  'gomboc-ai/ensure-the-base-image-uses-a-non-latest-version-tag';
const IAC_FILE = path.join(CODE_DIR, 'docker/Dockerfile');
const WORKSPACE_FILE_KEY = '/workspace/Dockerfile';

describe('Docker integration — non-latest base image tag rule', () => {
  let tempDir = '';
  let storageDir = '';
  ...
  it('Tier 1: replaces latest tag with pinned placeholder', async () => {
    const result = await runSingleRuleFix();
    const modified = result.modifiedFiles[WORKSPACE_FILE_KEY];
    expect(modified).toContain('FROM nginx:USER_INPUT');
  });
  ...
  it('Tier 3: converter emits diagnostic payload for Dockerfile', async () => {
    const result = await runSingleRuleFix();
